...
# class CSVM(nn.Module):
#     def __init__(self, num_class, q_num_bit, qat=False):
#         super(CSVM, self).__init__()
#         self.batch_size = 64
#         input_size = 784  # (dimension of image 28 * 28)
#         self.svm_model = CLinear(
#             input_size, num_class, q_num_bit=q_num_bit, qat=True)

#     def forward(self, x, labels):
#         x = self.svm_model(x)
#         x = torch.sum(torch.clamp(1 - x.t()*labels, min=0))/self.batch_size
#         return x

#     def quantize(self, if_quantize):
#         self.svm_model.quantize(if_quantize)

# def _svm_builder(num_class, q_num_bit, pretrained=True, name='SVM', qat=False):

#     model = CSVM(num_class, q_num_bit, qat=qat)
...
#             with open(pretrained, 'rb') as f:
#                 state_dict = torch.load(f, map_location='cpu')
#         model.svm_model.load_pretrained(state_dict, q_num_bit)
#         print('remained state dict', state_dict.keys())
...
def CResnet50(num_class, q_num_bit, pretrained=True, qat=False):
    ...
    return _resnet_builder(num_class, q_num_bit, CBottlneckBlock, [3,4,6,3], pretrained, 'resnet50', qat=qat)
...
# def CSVM8(num_class, q_num_bit, pretrained=True, qat=False):
#     return _svm_builder(num_class, q_num_bit, pretrained, 'resnet18', qat=qat)
