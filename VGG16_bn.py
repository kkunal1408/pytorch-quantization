...
from quantization_functions import quant_aware_vgg16_model
import tqdm
...
# %%
SAVE_DIR = 'checkpoint/imagenette_vgg16'
...
print(base_model)
mod = list(base_model.classifier.children())
mod.pop()
mod.append(torch.nn.Linear(4096, N_CLASS))
...
if base_model == 1:
    ...
    print(f'{SAVE_DIR}/base_model/model_weights.pt vgg16 base model weights')
...
if bit_8_model == 1:
    ...
    c_base_model = quant_aware_vgg16_model.Cvgg16(num_class=10, q_num_bit=8, qat=False,
                                                      pretrained=f'{SAVE_DIR}/base_model/model_weights.pt')
    c_base_model.quantize(True)

    # Forward pass to have quantized weights
    print("testing loaded quantized vgg16 model")
    train_loop.test_model(train_loader, c_base_model)

    # %%
    # Convert to quantized model
    print("quantizing model")
    q_base_model = post_training_quant_model.Qvgg16(num_class=10)
...
if bit_7_model == 1:
    ...
    # %%
    # Convert base model to a custom quantization layer with the trained weights
    c_base_model = quant_aware_vgg16_model.Cvgg16(num_class=10, q_num_bit=7, qat=False,
                                                      pretrained=f'{SAVE_DIR}/base_model/model_weights.pt')
    c_base_model.quantize(True)
...
if bit_6_model == 1:
    ...
    # %%
    # Convert base model to a custom quantization layer with the trained weights
    c_base_model = quant_aware_vgg16_model.Cvgg16(num_class=10, q_num_bit=6, qat=False,
                                                      pretrained=f'{SAVE_DIR}/base_model/model_weights.pt')
    c_base_model.quantize(True)
...
if bit_6_model == 1:
    ...
    # %%
    # Convert base model to a custom quantization layer with the trained weights
    c_base_model = quant_aware_vgg16_model.Cvgg16(num_class=10, q_num_bit=5, qat=False,
                                                      pretrained=f'{SAVE_DIR}/base_model/model_weights.pt')
    c_base_model.quantize(True)
...
if bit_4_model == 1:
    ...
    # %%
    # Convert base model to a custom quantization layer with the trained weights
    c_base_model = quant_aware_vgg16_model.Cvgg16(num_class=10, q_num_bit=4, qat=False,
                                                      pretrained=f'{SAVE_DIR}/base_model/model_weights.pt')
    c_base_model.quantize(True)
...
if single_layer_quantization_model1 == 1:
    ...
    for layer_no, layer_name in enumerate(model_bits.keys()):
        ...
        print(layer_name)
        c_base_model = quant_aware_vgg16_model.Cvgg16(
